...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	...
	var defaults = {
		...
		configUrl: './config/filemanager.config.default.json',
	...
	var initialize = function () {
		...
		if (_url_.param('window_close')) {

		    if (_url_.param("window_close")==1) {

		        fmModel.headerModel.closeButton(true);
		        fmModel.headerModel.closeButtonOnClick = function () {

		            parent.document.body.style.overflowY = "scroll";
		            parent.document.getElementById("fm-iframe").remove();
	...
	var isAuthorizedFile = function(filename) {
		...
		if (config.security.extensions.ignoreCase) {
		    if(config.security.extensions.policy === 'ALLOW_LIST') {
		        if ($.inArrayInsensitive(ext, config.security.uploadRestrictions) !== -1) return true;
		    }
		    ...
		    if(config.security.extensions.policy === 'DISALLOW_ALL') {
		        if ($.inArray(ext, config.security.uploadRestrictions) === -1) return true;
		    }
		}
	...
	var selectItem = function(resourceObject) {
		...
		if(_url_.param('CKEditor')) {
			if (window.opener) {
			    ...
			    window.opener.CKEDITOR.tools.callFunction(_url_.param('CKEditorFuncNum'), resourceObject.attributes.path);
			} else {
				// Modal (in iframe)
			    parent.CKEDITOR.tools.callFunction(_url_.param('CKEditorFuncNum'), resourceObject.attributes.path);
				parent.CKEDITOR.tools.callFunction(_url_.param('CKEditorCleanUpFuncNum'));
			}

			window.close();
		}
		if (_url_.param('customwindow'))
		{
		  
		   // window.opener.postMessage(
           //    {
           //        source: 'customwindow',
           //        dataobject: resourceObject
            ...
            contextWindow.postMessage(
            	{
                    ...
                    isimage:isImageFile(previewUrl),
	...
	var renameItem = function(resourceObject) {
		var doRename = function(e, ui) {
			...
			if (isFile(oldPath) && !isAuthorizedFile(givenName)) {
				...
				if(config.security.extensions.policy === 'ALLOW_ALL') {
				    str += '<p>' + lg('ALLOWED_FILE_TYPE').replace('%s', config.security.uploadRestrictions.join(', ')) + '.</p>';
				}
				if(config.security.extensions.policy === 'DISALLOW_ALL') {
				    str += '<p>' + lg('DISALLOWED_FILE_TYPE').replace('%s', config.security.uploadRestrictions.join(', ')) + '.</p>';
				}
				fm.error(str);
				return;
			}
