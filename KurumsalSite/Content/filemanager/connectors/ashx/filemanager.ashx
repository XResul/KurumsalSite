...
public class filemanager : IHttpHandler 
{
    ...
    public string[] imgExtensions = new string[] { ".jpg", ".png", ".jpeg", ".gif", ".bmp",".webp" }; // Only allow this image extensions. [string]
	public long size = 0L;
	public int files = 0;
	...
	private string HandleUpload(string path, HttpFileCollection uploadfiles)
	{
        ...
        foreach (string file in uploadfiles)
        {
            ...
            string pth = Path.Combine(path, Path.GetFileName(FileName));
            FileInfo fin = new FileInfo(context.Server.MapPath(pth));
            FileName = Alibrary.ExtensionsClass.ToUrlString(Path.GetFileNameWithoutExtension(fin.Name)) + uzanti;
               int sy = 1;
               while (File.Exists(context.Server.MapPath(Path.Combine(path, Path.GetFileName(FileName)))))
                {
                    sy++;
                    string stm = Path.GetFileNameWithoutExtension(FileName);
                    FileName = stm + "_" + sy + uzanti;
                }
            ...
            if (IsImage(fi))
            {
                ...
                string thumb = context.Server.MapPath(path + "thumb/");
                ...
                System.Drawing.Image ym = System.Drawing.Image.FromStream(hpf.InputStream);
                Alibrary.GraphicClass.ImageBoyut im = new Alibrary.GraphicClass.ImageBoyut();

                Guid gd = Guid.NewGuid();
                if (ym.Width > 1200)
                {
                    System.Collections.Generic.List<System.Drawing.Image> lst = im.Boyutla(ym, 1200, 600);

                    if (uzanti.ToLower() == ".jpg" || uzanti.ToLower() == ".jpeg")
                    {
                        im.saveJpeg(context.Server.MapPath(Path.Combine(path, Path.GetFileName(FileName))), lst[0], 100L);
                        im.saveJpeg(context.Server.MapPath(Path.Combine(thumb, Path.GetFileName(FileName))), lst[1], 100L);

                    }
                    if (uzanti.ToLower() == ".png")
                    {
                        im.savePng(context.Server.MapPath(Path.Combine(path, Path.GetFileName(FileName))), lst[0], 100L);
                        im.savePng(context.Server.MapPath(Path.Combine(thumb, Path.GetFileName(FileName))), lst[1], 100L);

                    }
                }
                else
                {
                    System.Collections.Generic.List<System.Drawing.Image> lst = im.Boyutla(ym, ym.Width, 600);
                    if (uzanti.ToLower() == ".jpg" || uzanti.ToLower() == ".jpeg")
                    {
                    ...
                    hpf.SaveAs(context.Server.MapPath(Path.Combine(path, Path.GetFileName(FileName))));

                }



                if (uzanti.ToLower() == ".gif")
                {
                    hpf.SaveAs(context.Server.MapPath(Path.Combine(path, Path.GetFileName(FileName))));
                    hpf.SaveAs(context.Server.MapPath(Path.Combine(thumb, Path.GetFileName(FileName))));
    ...
    private string Delete(string path) 
    {
        ...
        else
        {
          ...
          string thm=  fileInfo.Directory.FullName + "/thumb/" + fileInfo.Name;
